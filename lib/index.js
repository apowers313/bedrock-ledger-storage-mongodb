...
api.add = (meta = {}, options = {}, callback) => {
  ...
  assert.optionalArrayOfString(options.services, 'options.services');
  ...
  async.auto({
    ...
    // ensure that all the specified services are registered, no NotFoundError
    // and that the service type is valid
    services: callback => {
      try {
        for(const service of services) {
          const svc = api.use(service);
          if(svc.type !== 'service') {
            return callback(new BedrockError(
              'The specified service plugin must have a type of `service`.',
              'InvalidAccessError', {httpStatusCode: 400, public: true, service}
            ));
          }
        }
      } catch(e) {
    ...
    insert: ['generateUuid', 'services', (results, callback) => {
      // insert the ledger
      const now = Date.now();
      const record = {
    ...
    // storage plugins add indexes to collections by providing an `index` method
    extendIndexes: ['createCollectionIndexes', (results, callback) => {
      ...
      async.eachSeries(services, (service, callback) => {
        async.auto({
          service: callback => {
            let svc;
            try {
              svc = injector.use(service);
            } catch(e) {
              return callback(e);
            }
            callback(null, svc);
          },
          index: ['service', (results, callback) => {
            if(!results.service.api.index) {
              return callback();
            }
            const cbService = callbackify(results.service.api.index);
            cbService(options, callback);
          }]
        }, callback);
      }, callback);
    }],
    ledgerStorage: ['extendIndexes', (results, callback) => {
      const {blockCollection, eventCollection, operationCollection} =
        results.generateUuid;
      const lsOptions = {
        blockCollection:
    ...
    extend: ['ledgerStorage', (results, callback) => _extendLedgerStorage(
      {ledgerStorage: results.ledgerStorage, services}, callback)],
...
api.get = (storageId, options, callback) => {
  async.auto({
    ...
    extend: ['ledgerStorage', (results, callback) => {
      ...
      const {services} = results.find.ledger;
      _extendLedgerStorage({ledgerStorage, services}, callback);
    }],
  }, (err, results) => err ? callback(err) : callback(null, results.extend));
};
...
function _extendLedgerStorage({ledgerStorage, services}, callback) {
  if(services.length === 0) {
  ...
  async.eachSeries(services, (service, callback) => {
    async.auto({
      ...
      extend: ['service', (results, callback) => {
        const {service} = results;
        if(!service.api.storage) {
        ...
        const validStorageApis = _.intersection(
          ...
          Object.keys(service.api.storage));
        validStorageApis.forEach(type => {
          Object.keys(service.api.storage[type]).forEach(method => {
            ledgerStorage[type][method] =
              service.api.storage[type][method].bind(ledgerStorage[type]);
          });
        });
        callback();
      }]
